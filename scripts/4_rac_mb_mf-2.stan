functions {
    real triple_hybrid(int num_trials, 
                       ...
                       array[] int s2, 
                       array[] int reward,
                       real alpha1, real alpha2, real beta1, real p, real w, real w_rac) {
        real log_lik = 0;
        ...
        for (t in 1:num_trials) {
            ...
            for (a in 1:2)
                Q[a] = w * Q_MB[a] + 
                       (1 - w) * ((1 - w_rac) * (Q_MF[a]) + w_rac * Q_CUE[a]);
...
parameters {
    real<lower=0, upper=1> alpha1;        // learning rate for second stage
